...
The most up-to date version of this document is available at:
    ...
    https://github.com/pooler/cdn-electrum-server/blob/master/HOWTO.md
...
document, we assume that user is called 'cdnelectrum'). We also assume the
cdnelectrum user has sudo rights, so we use '$ sudo command' when we need to.
...
you also need to run canadaecoind and keep a copy of the full blockchain, 
which is roughly 4 GB in April 2014. If you have less than 2 GB of RAM 
make sure you limit canadaecoind to 8 concurrent connections. If you have more 
...
### Step 1. Create a user for running canadaecoind and Electrum server

This step is optional, but for better security and resource separation I
suggest you create a separate user just for running `canadaecoind` and Electrum.
We will also use the `~/bin` directory to keep locally installed files
(others might want to use `/usr/local/bin` instead). We will download source
code files to the `~/src` directory.

    $ sudo adduser cdnelectrum --disabled-password
    $ sudo apt-get install git
    $ sudo su - cdnelectrum
    $ mkdir ~/bin ~/src
    $ echo $PATH

If you don't see `/home/cdnelectrum/bin` in the output, you should add this line
to your `.bashrc`, `.profile`, or `.bash_profile`, then logout and relogin:

    PATH="$HOME/bin:$PATH"
    $ exit

### Step 2. Download canadaecoind

We currently recommend canadaecoind 0.10.1.3 or 0.8.7.5 stable.

If you prefer to compile canadaecoind, here are some pointers for Ubuntu:

    $ sudo apt-get install make g++ python-leveldb libboost-all-dev libssl-dev libdb++-dev pkg-config automake libtool
    $ sudo su - cdnelectrum
    $ cd ~/src && git clone https://github.com/canadaecoin-project/canadaecoin.git -b master-0.10
    $ cd cdnelectrum
    $ ./autogen.sh
    $ ./configure --disable-wallet --without-miniupnpc
    $ make
    $ strip src/canadaecoind src/canadaecoin-cli src/canadaecoin-tx
    $ cp -a src/canadaecoind src/canadaecoin-cli src/canadaecoin-tx ~/bin

### Step 3. Configure and start canadaecoind

In order to allow Electrum to "talk" to `canadaecoind`, we need to set up an RPC
username and password for `canadaecoind`. We will then start `canadaecoind` and
wait for it to complete downloading the blockchain.

    $ mkdir ~/.canadaecoin
    $ $EDITOR ~/.canadaecoin/canadaecoin.conf

Write this in `canadaecoin.conf`:
...
If you have an existing installation of canadaecoind and have not previously
set txindex=1 you need to reindex the blockchain by running

    $ canadaecoind -reindex

If you already have a freshly indexed copy of the blockchain with txindex start `canadaecoind`:

    $ canadaecoind

Allow some time to pass, so `canadaecoind` connects to the network and starts
downloading blocks. You can check its progress by running:

    $ canadaecoin-cli getinfo

Before starting the electrum server your canadaecoind should have processed all
blocks and caught up to the current height of the network (not just the headers).
You should also set up your system to automatically start canadaecoind at boot
time, running as the 'cdnelectrum' user. Check your system documentation to
...
We will download the latest git snapshot for Electrum to configure and install it:
    ...
    $ cd ~
    $ git clone https://github.com/pooler/cdn-electrum-server.git
    $ cd cdn-electrum-server
    $ sudo configure
...
and further instructions from the cdn-electrum full archival server foundry at:
http://foundry.cdn-electrum.org/leveldb-dump/
...
Electrum reads a config file (/etc/cdn-electrum.conf) when starting up. This
file includes the database setup, canadaecoind RPC setup, and a few other
options.

The "configure" script listed above will create a config file at /etc/cdn-electrum.conf
...
The "configure" script will take care of this and ask you to create a user for running cdn-electrum-server.
If you're using user cdnelectrum to run electrum and have added it manually like shown in this HOWTO run 
the following code to add the limits to your /etc/security/limits.conf:

     echo "cdnelectrum hard nofile 65536" >> /etc/security/limits.conf
     echo "cdnelectrum soft nofile 65536" >> /etc/security/limits.conf

Two more things for you to consider:

1. To increase security you may want to close canadaecoind for incoming connections and connect outbound only

2. Consider restarting canadaecoind (together with cdn-electrum-server) on a weekly basis to clear out unconfirmed
...
The magic moment has come: you can now start your Electrum server as root (it will su to your unprivileged user):
    ...
    # cdn-electrum-server start

Note: If you want to run the server without installing it on your system, just run 'run_cdn_electrum_server" as the
unprivileged user.
...
If you want to stop Electrum server, use the 'stop' command:
    ...
    # cdn-electrum-server stop


If your system supports it, you may add cdn-electrum-server to the /etc/init.d directory. 
...
safely whenever your machine is rebooted.
    ...
    # ln -s `which cdn-electrum-server` /etc/init.d/cdn-electrum-server
    # update-rc.d cdn-electrum-server defaults
...
Canada eCoins to confirm that everything is working properly.

### Step 13. Join us on IRC, subscribe to the server thread

Say hi to the dev crew, other server operators, and fans on
irc.freenode.net #cdn-electrum and we'll try to congratulate you
on supporting the community by running an cdn-electrum node.

If you're operating a public cdn-electrum server please subscribe
to the following mailing list:
https://groups.google.com/forum/#!forum/cdn-electrum-server
It'll contain announcements about important updates to cdn-electrum
